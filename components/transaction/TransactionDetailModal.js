    ...
    return (
        <Modal modalOpen={modalOpen} setModalOpen={setModalOpen}>
            <div className="space-y-20">
                <TransactionProfile name={currentTransaction?.to.name} handle={currentTransaction?.to.handle} avatar={currentTransaction?.to.avatar} verified={currentTransaction?.to.verified} />
    ...
    return (
        <div className="flex flex-col items-center space-y-3">
            ...
            <div className="flex flex-col items-center space-y-1">
                ...
                <p className="text-sm font-light text-gray-600">Payment to ${handle}</p>
            </div>
        </div>
    )
}
    ...
    return (
        <div className="flex flex-col items-center justify-center space-y-4">
            ...
            <div className="flex flex-col items-center text-gray-400">
                <p>{description}</p>
                <p>
                    {format(transactionDate, 'MMM d')} at {format(transactionDate, 'h:mm aa')}
                </p>
            </div>
        </div>
    )
        ...
        <div className="space-y-1">
            {Object.entries(metadata).map(([title, data], index) => (
                <div key={index} className="flex justify-between">
                    ...
                    <p className="font-medium text-gray-400">{data}</p>
                </div>
            ))}
        </div>
    )
